...
def Fetch_quizzes_by_user(user_id):
    try:
        collection=db["quizzes"]
        quizzes=list(collection.find({"generated_by": user_id}))
        if not quizzes:
            return "error : Quiz not found"
        else :
            for quiz in quizzes:
...
def get_quiz_attempts_by_student_id(student_id):
    try:
        collection = db["QuizAttempts"]
        attempts = list(collection.find({"studentId": student_id}))
        ...
        return {"error": f"Error fetching quiz attempts by studentId: {str(e)}"}
